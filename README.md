...
```
In this instance, the Antigravity code trains a transformer architecture of a one layer transformer on the problem of modular multiplication ((a x b) mod 43). The input data consists of 35% of all possible pairs while the rest 65% constitute test data. The architecture involves learning the token embeddings, positional embeddings, using self-attention, and MLP blocks. In addition, optimization is carried out by means of AdamW and heavy weight decay to facilitate grokking and algorithmic simplicity. Throughout the training process, the code computes train accuracy, test accuracy, and test loss through tens of thousands of training steps with the focus on monitoring delayed increase in test accuracy, which indicates grokking. Finally, after good generalization performance is attained, mechanistic interpretability analysis is done of the embeddings to extract an algorithm learned by the transformer. This is achieved through the extraction and projection of the embedding vectors of the non-zero modular elements using PCA to 2 dimensions. The first plot colored embeddings with respect to their numerical values, which was largely disorganized and random in nature, proving that the model did not organize numbers by their magnitude. However, the second plot colored embeddings based on the discrete logarithm with respect to a primitive root modulo 43, resulting in a perfect circle shape. This means that the transformer model had learned how to encode modular multiplication as an operation in the exponents in a circular form. Put another way, instead of learning how to solve the task by rote memorization of the multiplication table, it found out a secret circuit, whereby it encodes numbers as points on the circle based on their discrete logarithm, performs addition in that space, and then decodes the output again to the modular space.
<img width="4200" height="1950" alt="image" src="https://github.com/user-attachments/assets/5cc65375-433c-4a2c-acea-bd4398c8ba02" />
